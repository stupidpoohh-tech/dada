...
| `say_sent` | 제작자에게 한마디를 남겼다 | `kind`(hi·good·idea) |
...
"mascot": {
  ...
  // "item": "playgrown"       // 열 문서가 생기면 다시 채운다. 있으면 링크가 이긴다
  "column": {                  // 문서가 없는 동안 대신 펴는 글
    "icon": "📰",
    "title": "제3의 공간",
    "label": "리서치",
    "notes": ["...", "..."],   // 개조식 문단. 앞의 ✔는 CSS(::before)가 세운다
    "quote": {                 // 사람 말은 색 상자로 따로 뜬다
      "lines": ["...", "..."],
      "by": "이승은(26세, 프리랜서, 미국 소재 대학교 및 직장 경험 있음)"
    },
    "soon": "PlayGrown 기록을 전시 세션으로 정리하고 있어요. ..."   // 맨 아래 한 줄
  }
}
```
...
다원하는 다원 👋        ← 푸터에 남는 것은 이것뿐
   ↓ 누르면
👋 그냥 인사   👍 좋았어요   💡 할 말 있어요     ← 창 안
```

**배치를 두 번 옮겼다.** 처음에는 버튼 하나로 접어 두고 펴면 세 줄짜리 폼
(말·이름·답장받을곳)이 나왔고, 다음에는 그림 셋을 푸터에 늘어놓았다.
둘 다 나가는 길에 마주치기엔 자리를 너무 많이 먹었다 — **이름 옆의 손짓 하나면
「말을 걸 수 있다」는 뜻은 다 서고**, 무엇으로 인사할지는 창에서 고르면 된다.
...
인사와 좋았어요는 **누르면 그대로 가고**, 「할 말 있어요」만 적는 칸을 편다 —
말이 있어야 뜻이 사는 하나뿐이라서다.

서버는 `kind`(`hi`·`good`·`idea`)와, 할 말에만 따라오는 `text`를 받는다.
**그림만 눌러도 한마디다** — 글이 없다고 빈손으로 보지 않는다.
목록에 없는 `kind`는 버린다(글이 있으면 글은 살린다).
...
- **남기고 나면 그림도 칸도 치운다.** 빈 칸을 다시 내밀면 방금 남긴 것이 안 갔나
  싶어진다. 이때 알림 문구(`#sayNote`)는 **둘 다의 바깥**에 있어야 한다 — 폼 안에
  뒀다가 폼과 함께 사라져 고맙다는 말이 안 뜬 적이 있다
