...
- **사각형으로 못 오리는 건물은 `shape`를 쓴다.** 학교는 지붕 위 양옆에 나무가, 미술관은
  좌우 날개가 사각형 밖으로 걸린다. `districts[].shape`에 지도 % 좌표의 다각형을 넣으면
  복제본에 `clip-path`로 실루엣만 남는다. 좌표가 곧 지도 좌표라 변환이 필요 없다
  (복제본이 지도 원본 크기라서 `%`가 지도 좌표와 같다). `building`은 그 다각형의 바깥 상자로 둔다.
