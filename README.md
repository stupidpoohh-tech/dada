...
건물·공원 사람들·"나" 캐릭터·화살표는 모두 2.4초 한 박자로 움직인다. 하나만 다른 리듬을
주면 마을이 어수선해 보인다.

**눌러볼 곳마다 작은 화살표를 하나씩 띄운다** (`makeTags()`). 폰에서 건물 뾰잉은 5px,
우편함은 0.2px뿐인 데다 자동차·새·구름·사람이 전부 움직여 그 5px이 배경 소음에 묻힌다 —
움직임만으로는 무엇이 눌리는지가 드러나지 않는다. 그래서 「여기」를 직접 가리킨다.

- 크기는 **우편함만 하게** (지도 폭 2% × 높이 3.2%). 이보다 크면 마을이 아니라 표지판이 된다
- 색은 핫핑크(`--tag`). 지도의 초록·베이지에 없는 색이라야 눈이 먼저 잡는다.
  빛나는 효과는 `drop-shadow`를 겹쳐 낸다 — `clip-path`로 잘린 모양도 실루엣을 따라간다
- 레이어(`.tags`)는 `pointer-events: none`이라 버튼 위를 덮어도 클릭을 먹지 않는다.
  새·구름(`.sky`)보다 위(`z-index: 7`)에 둬서 무엇에도 가리지 않는다
- **위치 요소와 변형 요소를 나눈다.** 자리는 바깥 `.tag`가 잡고 둥둥 뜨는 것은 안쪽 `<i>`뿐이다

**가리키는 곳은 구역 블록이 아니라 반응하는 것이다.** 성균관은 블록 위쪽이 비어 있고,
공원은 땅이 아니라 사람들이 반응한다 — 블록 한가운데를 가리키면 빈 잔디를 가리키게 된다.
「나」와 공원 사람들은 좌표만으로 머리 위를 못 찾는다. 그림이 발밑에 앉아 있어서
(`translate(-50%, -100%)`) 데이터 좌표를 그대로 쓰면 화살표가 배 위에 박힌다 → `syncTags()`가
그려진 뒤 실측해서 얹는다.

- **잴 때 끄는 것은 `transform`이 아니라 `animation`이다.** 사람들과 「나」는 자리 잡는
  transform과 흔들리는 transform이 한 속성에 겹쳐 있어, 뾰잉 복제본처럼 `transform: none`으로
  누르면 자리가 통째로 어긋난다. `animation`만 끄면 CSS 규칙의 transform이 남아 제자리에 선다
- 그림이 늦게 오면 잰 키가 0이라 같은 증상이 난다. `init()`이 `fetch`를 기다리는 사이
  `window`의 `load`가 이미 지나가 있을 수 있어 그쪽에 기대면 늦는 날과 안 늦는 날이 갈린다 —
  **`ResizeObserver`로 크기가 잡히는 순간 다시 얹는다** (`watchTags()`)
...
`.sky`(구름·새)는 `z-index: 6`으로 건물 위를 지난다. 가리키는 화살표(`.tags`)는 그보다 위인
`7` — 새가 화살표를 가리면 가리키는 구실을 못 한다. 대신 `.map`에 `isolation: isolate`를 줘
그 z-index들이 지도 밖(구역 패널·아트북 팝업)까지 넘치지 않게 가둔다.
