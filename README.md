...
tests/smoke.mjs       회귀 테스트 121개 (npm test)
```
...
`tests/smoke.mjs`의 121개 검사는 **전부 실제로 한 번씩 깨졌던 것**이다. 커버리지를 채우려고
...
| 지붕 위 확성기 | **한 박자에 두 번 앞으로 내지른다** | 2.4초 |
...
**우편함이 기울 때 크기를 함께 키우는 이유**가 있다. 이건 지도를 오려낸 복제본이라
제자리에서 벗어나면 뒤에 있는 원본 우편함이 드러나 이중으로 보인다. 회전으로 밀려나는
만큼(높이 × sin7°) 덮으려면 가로로 그만큼 넉넉해야 한다 — 그래서 `scale(1.34, 1.14)`다.
...
밀리는 게 제 몸짓**이다. 축은 지붕에 붙은 받침(`transform-origin: 34% 92%`)이고,
...
지붕 위에 얹혀 있지만 **세모집 클릭을 가로채지 않는다.** 각자 판정 영역이 따로다 —
