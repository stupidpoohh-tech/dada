...
tests/smoke.mjs       회귀 테스트 114개 (npm test)
```
...
`tests/smoke.mjs`의 114개 검사는 **전부 실제로 한 번씩 깨졌던 것**이다. 커버리지를 채우려고
...
집 테마송이 나올 자리다. 지금은 **누를 수 없는 풍경**이다 — 노래와 가사를 받기 전이라
열 것이 없고, 없는 것을 가리키는 문은 만들지 않는다(까마귀와 같은 규칙).
...
지붕 위에 얹혀 있지만 **세모집 클릭을 가로채지 않는다** (`pointer-events: none`).
회귀 테스트가 그것까지 확인한다 — 확성기를 올린 뒤에도 세모집을 누르면 카탈로그가 열린다.

**노래를 받으면** `speaker`에 `audio`와 `lyrics`를 더하고, `<span>`을 `<button>`으로
바꿔 재생과 가사 팝업을 붙인다. 자리·크기·박자는 그대로 두면 된다.

스프라이트는 한 장짜리라 프레임 맞춤이 필요 없어 `cut_sprites.py`가 자른다
(여러 장을 포개야 하는 까마귀·쪽지만 따로 나가 있다).
