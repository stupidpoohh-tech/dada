...
tests/smoke.mjs       회귀 테스트 78개 (npm test)
```
...
`tests/smoke.mjs`의 78개 검사는 **전부 실제로 한 번씩 깨졌던 것**이다. 커버리지를 채우려고
...
**뾰잉은 위에서 아래로 끊임없이 흐른다.** 마을을 지도 세로 위치로 위·가운데·아래
세 줄로 묶고(`bandOf`), 줄마다 한 박자(2.4초)의 약 12%인 **280ms씩 위상을 어긋내** 준다.
위 줄이 먼저 솟고 가운데, 아래가 뒤따라 파도가 내려온다.
처음 온 사람에게 이 지도는 그냥 그림이라 — 폰에서 뾰잉은 건물 5.1px, 우편함 0.2px뿐이고
자동차·새·구름·사람이 다 움직여 그게 배경 소음에 묻힌다 — 무엇이 눌리는지가
드러나지 않기 때문이다. **차례로 번지는 것이 「이것들은 한 세트」를 말해준다.**
표식(화살표·물결)을 하나씩 얹는 방식은 열 개가 되는 순간 두 번째 벽지가 된다.

- 줄은 **화면에 보이는 중심 y(%)** 로 가른다(경계 40 / 75). 구역 블록(`rect`)이 아니라
  건물·우편함 상자의 중심이다 — 성균관처럼 블록 위쪽이 비어 있는 곳이 다른 줄로 새어나간다
- 위상은 `startTime`을 밀어 만든다. `beatOrigin` 하나를 잡아 두고 모든 요소를
  `beatOrigin - (ROWS - 1 - row) * ROW_STEP`으로 되맞추므로(`syncBeat()`),
  팝오버를 여닫거나 hover가 풀려 애니메이션이 다시 시작돼도 파도가 그대로 유지된다
- **한 번 지나가고 끝나지 않는다.** 예전에는 첫 방문에만 한 번 인사(`greetTown()`)했는데,
  못 보고 지나치면 다시 볼 길이 없어 접었다. 지금은 상시 idle 자체가 파도다
...
**호버·뾰옹이 물러난 뒤에는 반드시 `syncBeat()`을 불러야 한다.** 잠깐 다른 애니메이션이
얹혔다 사라지면 idle이 0부터 다시 시작해 **그 요소만** 마을과 어긋난다 — 재보면 2.4초 주기에서
2~3초씩 벌어져 사실상 반대 위상이 된다. 그래서 팝오버를 닫을 때·호버가 풀릴 때 다시 맞춘다.

**기준은 처음 한 번 잡아둔 `beatOrigin` 하나다.** 그때그때 현재 시각으로 되맞추면 어긋남은
없어져도 **온 마을이 그 순간 한 번 움찔한다.** `beatOrigin`을 고정해 두고 줄마다 정해진
만큼만 빼면, 뒤늦게 합류한 요소가 이미 도는 파도에 조용히 끼어든다.
...
건물과 사람들은 같은 2.4초 박자로, 줄마다 280ms씩 어긋나 움직인다. 각각 다른 시점에 만들어져
수십 ms씩 제멋대로 벌어지므로 `syncBeat()`이 `startTime`을 맞춰준다 — 의도한 어긋남이 아닌
어긋남은 눈에 거슬린다.
