...
tests/smoke.mjs       회귀 테스트 77개 (npm test)
```
...
`tests/smoke.mjs`의 77개 검사는 **전부 실제로 한 번씩 깨졌던 것**이다. 커버리지를 채우려고
...
**첫 방문에는 마을이 한 번 인사한다** (`greetTown()`). 왼쪽 위에서 오른쪽 아래로
대각선(`x+y` 순)으로 번지며 구역이 차례로 크게 뛰고, 우편함이 맨 끝이다.
처음 온 사람에게 이 지도는 그냥 그림이라 — 폰에서 상시 뾰잉은 5px뿐이고
자동차·새·구름·사람이 다 움직여 그 5px이 배경 소음에 묻힌다 — 무엇이 눌리는지가
드러나지 않기 때문이다. **차례로 번지는 것이 「이것들은 한 세트」를 말해준다.**
표식(화살표·물결)을 하나씩 얹는 방식은 열 개가 되는 순간 두 번째 벽지가 된다.

- `localStorage`의 `dada-greeted`로 **한 번만**. 해시로 바로 들어온 경우엔 건너뛴다
- 인사가 idle을 밀어냈다 물러나면 그 요소만 0부터 다시 시작하므로,
  파도가 다 지나간 뒤 **`syncIdle()`로 온 마을을 되맞춘다**
- **파도가 도는 3초 남짓은 마을이 일부러 한 박자가 아니다.** 그래서 회귀 테스트의
  다른 검사들은 `asReturning()`으로 재방문자로 열어 파도를 건너뛴다
