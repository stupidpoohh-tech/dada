...
- **grid 칸 안의 `max-height: 100%`는 행 크기를 안 적으면 아무것도 안 막는다.**
  행이 `auto`라 그림만큼 커지고, 그러면 그림이 **제 높이를 자기 기준으로 재는 꼴**이
  된다. 안내서 카드가 그래서 잘렸는데 **1600px 넘는 화면과 가로로 돌린 폰에서만**
  드러났다 — 개발하며 쓰는 1440px에서는 멀쩡해 보인다. 길이를 직접 준다
  (`max-height: var(--card-h)`). **화면 크기에 딸린 것은 한 벌만 보고 넘기지 않는다.**
...
- **playwright로 마을을 열 때는 안내를 꺼 두고 연다.** 안내는 **올 때마다** 화면
  전체에 덮개를 깔아서, 안 그러면 누르는 것마다 덮개가 먹고 「지도가 죽었다」로만 보인다.
  `tests/smoke.mjs`의 `watch()`가 이미 심어 주니 그 헬퍼를 쓴다 (직접 열 때는
  `addInitScript`로 `localStorage['dada.introOff'] = '1'`). 안내 자체는 `first()`로 연다.
